...
(function() {
  ...
  $.fn.snappish = function(opts) {
    ...
    if (settings.currentSlideNum == undefined)
        currentSlideNum = 0;
    else
        currentSlideNum = settings.currentSlideNum;
    //alert(currentSlideNum + "\n" + numberOfSlides);
    ...
    if (settings.mousewheelEnabled) {
      $wrapper.on('mousewheel', function(e, delta, deltaX, deltaY) {
        if (inTransition || $(this).find('.active').hasClass('y-maps-hovered')) {
          return;
        }
        if (deltaY < 0) {
          return scroll('down');
    ...
    if (settings.swipeEnabled) {
      ...
      $wrapper.on('swipeup', function(e) {
        return $(this).find('.active').find('#y-maps').length ? false : scroll('down');
      });
      $wrapper.on('swipedown', function(e) {
        return $(this).find('.active').find('#y-maps').length ? false : scroll('up');
      });
    }
